...
app.get('/api/boards/:id', verifyToken, async (req, res) => {
  try { res.json(await Board.findById(req.params.id)); } catch (e) { res.status(500).send(e); }
});
...
app.get('/api/quotes', verifyToken, async (req, res) => {
  try { const quotes = await Quote.find().sort({ createdAt: -1 }); res.json(quotes); } 
  catch (e) { res.status(500).json({error: e.message}); }
});
